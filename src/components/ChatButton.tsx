      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }}
            ...
            className={`fixed bottom-20 md:bottom-24 ${isRtl ? "left-4 md:left-6" : "right-4 md:right-6"} z-50 w-[calc(100vw-32px)] md:w-[380px] max-h-[70vh] md:max-h-[560px] bg-background rounded-2xl shadow-2xl border border-border/50 flex flex-col overflow-hidden`}>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 max-h-[380px]">
              {messages.map((msg, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}
                  ...
                  <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm font-body leading-relaxed ${msg.role === "user" ? "bg-primary text-primary-foreground rounded-br-md" : "bg-secondary/30 text-foreground rounded-bl-md"
                    }`}>
